...
public class Testing {
    ...
    public static void main(String[] args) {
        ...
        long result = LongStream.range(1, RANGE).parallel().filter(Testing::divBy5).reduce(1, (a, b) -> a * b);
    ...
    static String unSort(String input) {
        ...
        List<String> filtered = Arrays.stream(separators).filter(s -> s.length() > 0).collect(Collectors.toList());
        Arrays.sort(words);
    ...
    static void findSum(int a, int b) {
        int sum = IntStream.range(a, b).map(x -> x).reduce(1, (left, right) -> left * right);
